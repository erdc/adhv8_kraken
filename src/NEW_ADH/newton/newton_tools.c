 ...
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void initialize_dirichlet_bc(SMODEL_SUPER *sm, SGRID *grid) {
 ...
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void update_function(SMODEL_SUPER *sm,SGRID *grid,SMAT *mat){
    int j,k;
 ...
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void increment_function(SMODEL_SUPER *sm,SGRID *grid,SMAT *mat){
    int j,k;
    ...
    for (j=0;j<grid->nelem3d;j++){
        for (k=0;k<sm.nSubMods3d[j];k++){
            ...
            //would this work for vector functions?
            //would it be better to put global residual outside inner loop if possible?
            sm.elem3d_physics[j][k]->fe_inc(sm,temp,grid,mat,j, 0.0, UNSET_INT, PERTURB_NONE, 0, DEBUG);
    ...
    for (j=0;j<grid->nelem2d;j++){
        for (k=0;k<sm.nSubMods2d[j];k++){
            //would this work for vector functions?
            sm.elem2d_physics[j][k]->fe_inc(sm,temp,grid,mat,j, 0.0, UNSET_INT, PERTURB_NONE, 0, DEBUG);
    ...
    for (j=0;j<grid->nelem1d;j++){
        for (k=0;k<sm.nSubMods1d[j];k++){
            //would this work for vector functions?
            sm.elem1d_physics[j][k]->fe_inc(sm,temp,grid,mat,j, 0.0, UNSET_INT, PERTURB_NONE, 0, DEBUG);
...
void get_residual_norms(double *resid_max_norm, double *resid_l2_norm, double *inc_max_norm,
         int *imax_node, int *iinc_node, int *include_node,
         int my_nnodes, int nnodes, int macro_nnodes, double *residual, double *sol
        ,int *nodal_var  
        ){
    
    int i, j, include_node_flag;
    int idof=0;
    double lolh = 1.0;
