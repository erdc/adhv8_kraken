...
int fe_sw2_body_resid(SMODEL_SUPER *mod, double *elem_rhs, int ie, double perturbation, int perturb_node, int perturb_var, int perturb_sign, int DEBUG) {
    ...
    global_to_local_dbl_cg(mod->sol, elem_head, elem2d->nodes, nnodes, PERTURB_U, mod->dof_map_local, mod->node_physics_mat, mod->node_physics_mat_id);
    //global_to_local_dbl_cg_2(mod->sol, elem_head, elem2d->nodes, nnodes, PERTURB_U, mod->node_physics_mat, mod->node_physics_mat_id);
    //printf("Nnodes %d\n",nnodes);
    double f[nnodes];
...
//    global_to_local_svect2d(sw2->older_vel, elem_older_vel, elem2d->nodes, nnodes);
//    
//    double elem_density[nnodes]; sarray_init_value_dbl(elem_density,nnodes,0.);
//    if (mod->flag.BAROCLINIC == 1) {
//        global_to_local_dbl(sw2->density, elem_density, elem2d->nodes, nnodes);
//        for (i=0; i<nnodes; i++) {
...
//    
//    // this is used to store later for tansport
//    for (i=0; i<nnodes; i++) {
...
//            
//            double elem_wse_grad_mag = svect2d_mag_safe(elem_grad_wse);
...
//            for (i=0; i<nnodes; i++) {
//                // mass contributions
//                rhs[i].c_eq = dt * one_2 * g * djac * ((mod->drying_lower_limit * elem_grad_wse.x) * grad_shp[i].x + (mod->drying_lower_limit * elem_grad_wse.y) * grad_shp[i].y);
//                
//                // momentum contribution
//                norm.x = svect2d_dotp(grad_unorm, elem_grad_u);
//                norm.y = svect2d_dotp(grad_vnorm, elem_grad_v);
//                rhs[i].x_eq = sh_cap_coef_u * dt * djac * norm.x * svect2d_dotp(grad_unorm, grad_shp[i]);
//                rhs[i].y_eq = sh_cap_coef_v * dt * djac * norm.y * svect2d_dotp(grad_vnorm, grad_shp[i]);
//            }
//            
//            for (i=0; i<nnodes; i++) {
//                mod->sw->d2->elem_rhs_dacont_extra_terms[i][ie] += rhs[i].c_eq; // cjt :: store
//                elem_rhs[i].c_eq += rhs[i].c_eq;
//                elem_rhs[i].x_eq += rhs[i].x_eq;
//                elem_rhs[i].y_eq += rhs[i].y_eq;
//            }
//            
//#ifdef _DEBUG
//            if (DEBUG == ON || DEBUG_LOCAL == ON) {
//                rhs_3dof("2D SW || SHOCK CAPTURING",nnodes, ie, elem2d->nodes, rhs);
...
//        for (i=0; i<nnodes; i++) {
//            rhs[i].c_eq = rhs_c_eq[i];
//            rhs[i].x_eq = rhs_x_eq[i];
//            rhs[i].y_eq = rhs_y_eq[i];
//        }
//    }
//    for (i=0; i<nnodes; i++) {
//        elem_rhs[i].c_eq += rhs[i].c_eq;
...
//#ifdef _DEBUG
//    if (DEBUG == ON || DEBUG_LOCAL == ON) {
//        rhs_3dof("2D SW || ADVECTION",nnodes, ie, elem2d->nodes, rhs);
//    }
//#endif
//    
//    /*!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//                    (-1.) * (1. + alpha), elem_rhs, "2D SW || TEMPORAL T(N)", DEBUG, DEBUG_LOCAL, mod->grid->wd_flag[ie]);
//    
//    // ++ t(n-1) terms +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//    fe_sw2_temporal(ie, elem2d, nnodes, elem_nds, djac, drying_lower_limit, elem_older_head, elem_older_vel, factor_older,
//                    (alpha / 2.), elem_rhs, "2D SW || TEMPORAL T(N-1)", DEBUG, DEBUG_LOCAL, mod->grid->wd_flag[ie]);
//    
//    /*!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//        }
//        rhs_3dof("2D SW || DIFFUSION",nnodes, ie, elem2d->nodes, rhs);
//    }
//#endif
//    
//    /*!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//    if (isTriangle == TRUE) {
//        integrate_triangle_phi_f(djac, t1, elem_friction_x, rhs_x_eq);
...
//    
//    for (i=0; i<nnodes; i++) {
//        elem_rhs[i].x_eq += rhs_x_eq[i];
//        elem_rhs[i].y_eq += rhs_y_eq[i];
//    }
//#ifdef _DEBUG
//    if (DEBUG == ON || DEBUG_LOCAL == ON) {
//        for (i=0; i<nnodes; i++) {
...
//            rhs[i].y_eq = rhs_y_eq[i];
//        }
//        rhs_3dof("2D SW || FRICTION",nnodes, ie, elem2d->nodes, rhs);
...
//        
//        for (i=0; i<nnodes; i++) {
//            elem_rhs[i].x_eq += one_3 * dt * stream_accel_vect.x * djac;
//            elem_rhs[i].y_eq += one_3 * dt * stream_accel_vect.y * djac;
//        }
//#ifdef _DEBUG
//        if (DEBUG == ON || DEBUG_LOCAL == ON) {
//            rhs_3dof("2D SW || VORTICITY",nnodes, ie, elem2d->nodes, elem_rhs);
//        }
//#endif
//    }
//    
...
//    if (isTriangle == TRUE) {
//        integrate_triangle_phi_f_g(djac, -t1, elem_head, elem_v, rhs_x_eq);
...
//            rhs[i].y_eq = rhs_y_eq[i];
//        }
//        rhs_3dof("2D SW || CORIOLIS",nnodes, ie, elem2d->nodes, rhs);
//    }
//#endif
//    
//    /*!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//            }
//            rhs_3dof("2D SW || SURFACE_STRESS",nnodes, ie, elem2d->nodes, rhs);
//        }
//#endif
//    }
//    
...
//                rhs[i].y_eq = 0;
//            }
//            rhs_3dof("2D SW || SOURCE",nnodes, ie, elem2d->nodes, rhs);
//        }
//#endif
//    }
//    
...
//            rhs_3dof("2D SW || PRESSURE",nnodes, ie, elem2d->nodes, rhs);
//        }
//#endif
//        dof3_add_replace_array(elem_rhs, rhs, nnodes);
//        
//        /*!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n
...
//#ifdef _DEBUG
//        if (DEBUG == ON || DEBUG_LOCAL == ON || debug.pressure || debug.rhs) {
//            rhs_3dof("2D SW || BODY FORCE",nnodes, ie, elem2d->nodes, rhs);
//        }
//#endif
//        dof3_add_replace_array(elem_rhs, rhs, nnodes);
//        
...
//                rhs[nd1].x_eq += dely * rhs_c_eq[0];
//                rhs[nd2].x_eq += dely * rhs_c_eq[1];
//                rhs[nd1].y_eq += delx * rhs_c_eq[0];
//                rhs[nd2].y_eq += delx * rhs_c_eq[1];
//            }
//        }
//#ifdef _DEBUG
//        if (DEBUG == ON || DEBUG_LOCAL == ON || debug.pressure || debug.rhs) {
//            rhs_3dof("2D SW || BOUNDARY PRESSURE",nnodes, ie, elem2d->nodes, rhs);
//        }
//#endif
//        dof3_add_replace_array(elem_rhs, rhs, nnodes);
//        
...
//#ifdef _DEBUG
//            if (DEBUG == ON || DEBUG_LOCAL == ON || debug.pressure || debug.rhs) {
//                rhs_3dof("2D SW || DENSITY PRESSURE",nnodes, ie, elem2d->nodes, rhs);
//            }
//#endif
//            dof3_add_replace_array(elem_rhs, rhs, nnodes);
//            
...
//#ifdef _DEBUG
//            if (DEBUG == ON || DEBUG_LOCAL == ON || debug.pressure || debug.rhs) {
//                rhs_3dof("2D SW || DENSITY BODY FORCE PRESSURE",nnodes, ie, elem2d->nodes, rhs);
//            }
//#endif
//            dof3_add_replace_array(elem_rhs, rhs, nnodes);
//            
...
//#ifdef _DEBUG
//            if (DEBUG == ON || DEBUG_LOCAL == ON || debug.pressure || debug.rhs) {
//                rhs_3dof("2D SW || DENSITY BOUNDARY PRESSURE",nnodes, ie, elem2d->nodes, rhs);
//            }
//#endif
//            dof3_add_replace_array(elem_rhs, rhs, nnodes);
//        }
...
//        double tau = fe_get_supg_tau_sw(nnodes, elem_nds, g_factor, elem_h_wd_avg_denom, elem_vel_wd_avg.x, elem_vel_wd_avg.y,0.,grad_shp_x,grad_shp_y,NULL,area * factor_old, mod->tau_pg, 2, 2, 2);
//        
...
//            dof3_init_array(dfdt, NDONTRI); // cjt :: note: older version of AdH all use current velocity here ... looks wrong
//            vars[0] = 1;
//            vars[1] = 1;
...
//            c_eq = dhdt + advection[0].c_eq + factor * elem_water_source * djac * dt;
//            x_eq = dudt + advection[0].x_eq + (x_pressure + x_friction/elem_h_wd_avg_denom) * djac + (x_coriolis - total_stress.x / elem_h_wd_avg_denom) * djac * factor * dt;
//            y_eq = dvdt + advection[0].y_eq + (y_pressure + y_friction/elem_h_wd_avg_denom) * djac + (y_coriolis - total_stress.y / elem_h_wd_avg_denom) * djac * factor * dt;
//            
...
//        }
//        for (i=0; i<nnodes; i++) {
//            mod->sw->d2->elem_rhs_dacont_extra_terms[i][ie] += rhs_c_eq[i];
//            elem_rhs[i].c_eq += rhs_c_eq[i];
//            elem_rhs[i].x_eq += rhs_x_eq[i];
//            elem_rhs[i].y_eq += rhs_y_eq[i];
//        }
//#ifdef _DEBUG
...
//#ifdef _DEBUG
//    if (DEBUG_LOCAL == ON) {
//        rhs_3dof("2D SW BODY || TOTAL RHS AFER DB",nnodes, ie, elem2d->nodes, elem_rhs);
//        if (DEBUG_PICKETS == ON) tl_check_all_pickets(__FILE__,__LINE__);
//    }
//    
...
//    }
//    if (DEBUG == ON || DEBUG_LOCAL == ON) {
//        rhs_3dof(string, nnodes, ie, elem2d->nodes, rhs);
//        Is_DoubleArray_Inf_or_NaN(rhs_c_eq, nnodes ,__FILE__ ,__LINE__);
//        Is_DoubleArray_Inf_or_NaN(rhs_x_eq, nnodes ,__FILE__ ,__LINE__);
//        Is_DoubleArray_Inf_or_NaN(rhs_y_eq, nnodes ,__FILE__ ,__LINE__);
//    }
