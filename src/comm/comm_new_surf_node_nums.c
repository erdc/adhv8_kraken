  ...
  )
{
  ...
  for (isd = 0; isd < npes; isd++)
    {
      ...
      messg_alloc(nnode_out[isd], sizeof(int), send_old_node_number + isd);
      messg_alloc(nnode_out[isd], sizeof(int), recv_new_node_number + isd);
    }
      ...
      if (nnodes_in[isd] > 0)
        {
          ...
          messg_alloc(recv_old_node_number.nitem, sizeof(int), send_new_node_number + recv_old_node_number.sd);
           
          /* parses the message */
          for (i = 0; i < recv_old_node_number.nitem; i++)
            {
               
              /* load each incoming node in the temporary global node number */
              tmp_node.resident_id = *(((int *) recv_old_node_number.buffer) + i);
  ...
  messg_buffer_free(&recv_old_node_number, g->smpi->ADH_COMM);
