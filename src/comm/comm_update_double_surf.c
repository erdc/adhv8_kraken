 ...
 */
void comm_update_double_surf(double *v,  /* the vector to be updated */
  ...
  for (i_processor = 0; i_processor < smpi->npes; i_processor++)
    {
      if (smpi->send_key_surf[i_processor].size > 0)
        {
          messg_buffer_alloc(nvalues * smpi->send_key_surf[i_processor].size, sizeof(double), smpi->send_msg + i_processor);
          ...
          for (ii = 0, icnt = 0; ii < smpi->send_key_surf[i_processor].size; ii++)
            { 
              for (jj = 0; jj < nvalues; jj++)
                {
                  bpntr[icnt++] = v[smpi->send_key_surf[i_processor].key[ii] * nvalues + jj];
                }
            }
