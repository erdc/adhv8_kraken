 ...
 */
void comm_update_int_surf(int *v,    /* the vector to be updated */
  ...
  for (i_processor = 0; i_processor < smpi->npes; i_processor++)
    {
      if (smpi->send_key_surf[i_processor].size > 0)
        {
          messg_buffer_alloc(nvalues * smpi->send_key_surf[i_processor].size, sizeof(int), smpi->send_msg + i_processor);
