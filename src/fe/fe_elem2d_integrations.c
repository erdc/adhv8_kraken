/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \file  fe_elem2d_integrations.c This file collects functions that integrate over 2d AdH 
 elements. NOTE: These integrations *ADD* to the integral, so be sure to zero when calling. */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
 ...
 *  \f$  \int_{0}^{1} \int_{0}^{1-\widehat{x}} c \, d\widehat{y}\,d\widehat{x} \f$
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_triangle_area(double djac, double c) {
    return (c * djac);
}
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, f(\widehat{x},\widehat{y})^2 \big) d\widehat{y}\,d\widehat{x}  \f$
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_f_f(SVECT *nd, double c, double *f) {
    ...
    t1 = (3*f1_2 + 3*f1*f2 + 3*f2_2 + (f1 + 2*f2)*f3 + f3_2 + (2*f1 + f2 + f3)*f4 + f4_2);
    t2 = (f1*f2 + 2*f2_2 + f2*f3 - (f1 + f3)*f4 - 2*f4_2);
    t3 = (3*f1_2 + 2*f1*f2 + f2_2 + (f1 + f2)*f3 + f3_2 + (3*f1 + f2 + 2*f3)*f4 + 3*f4_2);
    t4 = (f1_2 + 2*f1*f2 + 3*f2_2 + (f1 + 3*f2)*f3 + 3*f3_2 + (f1 + f2 + 2*f3)*f4 + f4_2);
    t5 = (2*f1_2 + f1*f2 - f2*f3 - 2*f3_2 + (f1 - f3)*f4);
    t6 = (f1_2 + f1*f2 + f2_2 + (f1 + 2*f2)*f3 + 3*f3_2 + (2*f1 + f2 + 3*f3)*f4 + 3*f4_2);
    
    return (c * one_36 * (-(t1*x2 - t2*x3 - t2*x4)*y1 + (t1*x1 - t4*x3 - t5*x4)*y2 - (t2*x1 - t4*x2 + t6*x4)*y3 - (t2*x1 - t5*x2 - t6*x3)*y4));
}
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_quadrilateral_gradPhi_dot_v(SVECT *nd, double c, SVECT2D *v, double *integral) {
    ...
    double con;
    double dy1, dy2, ty1, ty2, ty3, ty4;
    double dx1, dx2, tx1, tx2, tx3, tx4;
    ...
    con = one_12 * c;
    integral[0] += con * (-ty1*x2 + dy1*x3 + ty2*x4 + tx1*y2 - dx1*y3 - tx2*y4);
    integral[1] += con * ( ty1*x1 - ty3*x3 - dy2*x4 - tx1*y1 + tx3*y3 + dx2*y4);
    integral[2] += con * (-dy1*x1 + ty3*x2 - ty4*x4 + dx1*y1 - tx3*y2 + tx4*y4);
    integral[3] += con * (-ty2*x1 + dy2*x2 + ty4*x3 + tx2*y1 - dx2*y2 - tx4*y3);
}

/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following triangular integration:
 *  \f$  \int_{0}^{1} \int_{0}^{1-\widehat{x}} \big( c \, \mathbf{\nabla} \cdot (f(\widehat{x},\widehat{y}) \, \overline{\mathbf{v}})  \big)  d\widehat{y}\,d\widehat{x}\f$
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_triangle_grad_dot_f_vbar(SVECT2D *grad_shp, double djac, double c, double *f, SVECT2D vbar) {
   
    double dotp1 = vbar.x * grad_shp[0].x + vbar.y * grad_shp[0].y;
    double dotp2 = vbar.x * grad_shp[1].x + vbar.y * grad_shp[1].y;
    double dotp3 = vbar.x * grad_shp[2].x + vbar.y * grad_shp[2].y;
    double integral = dotp1 * f[0] + dotp2 * f[1] + dotp3 * f[2];
    return (integral * c * djac);
}
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_grad_dot_f_vbar(SVECT *nd, double c, double *f, SVECT2D vbar) {
    ...
    double integral = one_2 * c * ( (f2 - f4)*vbar.y*x1 - (f1 - f3)*vbar.y*x2 - (f2 - f4)*vbar.y*x3 + (f1 - f3)*vbar.y*x4 - (f2 - f4)*vbar.x*y1 + (f1 - f3)*vbar.x*y2 + (f2 - f4)*vbar.x*y3 - (f1 - f3)*vbar.x*y4 );
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following *quadratic basis triangular integration:
 *  \f$  \int_{0}^{1} \int_{0}^{1-\widehat{x}} \big( c \, \phi_i(\widehat{x},\widehat{y}) \, f(\widehat{x},\widehat{y})  \big)  d\widehat{y}\,d\widehat{x}\f$
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_triangle_phi_fquad(double djac, double constant, double *f, double *integral) {
    double t1 = djac * one_60 * constant;
    integral[0] += t1 * (2*f[0] -   f[1] -   f[2] + 8*f[3] + 4*f[4] + 8*f[5]);
    integral[1] += t1 * ( -f[0] + 2*f[1] -   f[2] + 8*f[3] + 8*f[4] + 4*f[5]);
    integral[2] += t1 * ( -f[0] -   f[1] + 2*f[2] + 4*f[3] + 8*f[4] + 8*f[5]);
}
...
/*! \brief  Peforms following quadrilateral integration:
 *  \f{eqnarray*}{
 *   integral_x &=& \int_{-1}^{1} \int_{-1}^{1} \bigg(c \, \frac{\partial u(\widehat{x},\widehat{y})}{x} \bigg) d\widehat{y}\,d\widehat{x} \\
 *   integral_y &=& \int_{-1}^{1} \int_{-1}^{1} \bigg(c \, \frac{\partial v(\widehat{x},\widehat{y})}{y} \bigg) d\widehat{y}\,d\widehat{x} \\
 *  \f}
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline SVECT2D integrate_quadrilateral_dv(SVECT *nd, double c, SVECT2D *v) {
    
    SVECT2D result;
    double x1, x2, x3, x4, y1, y2, y3, y4, u1, u2, u3, u4, v1, v2, v3, v4;
    
    extractVect2DQuad(v,&u1,&u2,&u3,&u4,&v1,&v2,&v3,&v4);
    extractNodesQuad(nd,&x1,&x2,&x3,&x4,&y1,&y2,&y3,&y4);
    
    double con =  one_2 * c;
    result.x =  con * (-(u2 - u4)*y1 + (u1 - u3)*y2 + (u2 - u4)*y3 - (u1 - u3)*y4);
    result.y =  con * ( (v2 - v4)*x1 - (v1 - v3)*x2 - (v2 - v4)*x3 + (v1 - v3)*x4);
...
/*! \brief  Peforms following triangular integration:
 *  \f{eqnarray*}{
 *   integral_x &=&  \int_{0}^{1} \int_{0}^{1-\widehat{x}} \bigg(c \, \frac{\partial \phi_i(\widehat{x},\widehat{y})}{x} \, f(\widehat{x},\widehat{y}) \, g(\widehat{x},\widehat{y})\bigg) d\widehat{y}\,d\widehat{x} \\
 *   integral_y &=&  \int_{0}^{1} \int_{0}^{1-\widehat{x}} \bigg(c \, \frac{\partial \phi_i(\widehat{x},\widehat{y})}{y} \, f(\widehat{x},\widehat{y}) \, g(\widehat{x},\widehat{y})\bigg) d\widehat{y}\,d\widehat{x}
 *  \f}
 *  \author  Corey Trahan, Ph.D.
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_triangle_dphi_f_g(SVECT2D *grad_shp, double djac, double c, double *f, double *g, double *integral_x, double *integral_y) {
    double con, ff1, ff2, ff3;
    
    ff1 = (2*f[0] +   f[1] +    f[2]);
    ff2 = (  f[0] + 2*f[1]  +   f[2]);
    ff3 = (  f[0] +   f[1]  + 2*f[2]);
    
    con = djac * one_12 * c * (ff1*g[0] + ff2*g[1] + ff3*g[2]);
    if (integral_x != NULL) {
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \mathbf{\nabla} \cdot (h(\widehat{x},\widehat{y}) \, \mathbf{v(\widehat{x},\widehat{y})})  \big) d\widehat{y}\,d\widehat{x}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: 2D strong convection type integrals
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_grad_dot_h_v(SVECT *nd, double c, SVECT2D *v, double *h) {
    
    double x1, x2, x3, x4, y1, y2, y3, y4, h1, h2, h3, h4, u1, u2, u3, u4, v1, v2, v3, v4;
    
    extractNodesQuad(nd,&x1,&x2,&x3,&x4,&y1,&y2,&y3,&y4);
    extractVect2DQuad(v,&u1,&u2,&u3,&u4,&v1,&v2,&v3,&v4);
    extractFunctionQuad(h,&h1,&h2,&h3,&h4);
    
    double result = one_6 * c * (((h2 - h4)*v1 + (h1 + 2*h2)*v2 - (h1 + 2*h4)*v4)*x1 - ((2*h1 + h2)*v1 + (h1 - h3)*v2 - (h2 + 2*h3)*v3)*x2 - ((2*h2 + h3)*v2 + (h2 - h4)*v3 - (h3 + 2*h4)*v4)*x3 + ((2*h1 + h4)*v1 - (2*h3 + h4)*v3 + (h1 - h3)*v4)*x4 - ((h2 - h4)*u1 + (h1 + 2*h2)*u2 - (h1 + 2*h4)*u4)*y1 + ((2*h1 + h2)*u1 + (h1 - h3)*u2 - (h2 + 2*h3)*u3)*y2 + ((2*h2 + h3)*u2 + (h2 - h4)*u3 - (h3 + 2*h4)*u4)*y3 - ((2*h1 + h4)*u1 - (2*h3 + h4)*u3 + (h1 - h3)*u4)*y4);
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \mathbf{\nabla} \cdot (h(\widehat{x},\widehat{y}) \, \overline{\mathbf{v}})  \big) d\widehat{y}\,d\widehat{x}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: 2D strong convection type integrals
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_grad_dot_h_vbar(SVECT *nd, double c, SVECT2D vbar, double *h) {
    
    double x1, x2, x3, x4, y1, y2, y3, y4, h1, h2, h3, h4;
    extractNodesQuad(nd,&x1,&x2,&x3,&x4,&y1,&y2,&y3,&y4);
    extractFunctionQuad(h,&h1,&h2,&h3,&h4);
    double ub = vbar.x, vb = vbar.y;
    
    double result = one_2 * c * (vb * ((h2 - h4) * x1 - (h1 - h3) * x2 - (h2 - h4) * x3 + (h1 - h3) * x4) -
                                 ub * ((h2 - h4) * y1 + (h1 - h3) * y2 + (h2 - h4) * y3 - (h1 - h3) * y4) );
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \mathbf{\nabla} \cdot (g(\widehat{x},\widehat{y}) \, h(\widehat{x},\widehat{y}) \, \mathbf{v(\widehat{x},\widehat{y})})  \big) d\widehat{y}\,d\widehat{x}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: 2D strong convection type integrals
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_grad_dot_h_g_v(SVECT *nd, double c, SVECT2D *v, double *h, double *g) {
    
    double x1, x2, x3, x4, y1, y2, y3, y4, g1, g2, g3, g4, h1, h2, h3, h4, v1, v2, v3, v4, u1, u2, u3, u4;
    extractNodesQuad(nd,&x1,&x2,&x3,&x4,&y1,&y2,&y3,&y4);
    extractFunctionQuad(h,&h1,&h2,&h3,&h4);
    extractFunctionQuad(g,&g1,&g2,&g3,&g4);
    extractVect2DQuad(v,&u1,&u2,&u3,&u4,&v1,&v2,&v3,&v4);
    
    double result = one_12 * c * ((((g2 - g4)*h1 + (g1 + g2)*h2 - (g1 + g4)*h4)*v1 + ((g1 + g2)*h1 + (g1 + 3*g2)*h2)*v2 - ((g1 + g4)*h1 + (g1 + 3*g4)*h4)*v4)*x1 - (((3*g1 + g2)*h1 + (g1 + g2)*h2)*v1 + ((g1 + g2)*h1 + (g1 - g3)*h2 - (g2 + g3)*h3)*v2 - ((g2 + g3)*h2 + (g2 + 3*g3)*h3)*v3)*x2 - (((3*g2 + g3)*h2 + (g2 + g3)*h3)*v2 + ((g2 + g3)*h2 + (g2 - g4)*h3 - (g3 + g4)*h4)*v3 - ((g3 + g4)*h3 + (g3 + 3*g4)*h4)*v4)*x3 + (((3*g1 + g4)*h1 + (g1 + g4)*h4)*v1 - ((3*g3 + g4)*h3 + (g3 + g4)*h4)*v3 + ((g1 + g4)*h1 - (g3 + g4)*h3 + (g1 - g3)*h4)*v4)*x4 - (((g2 - g4)*h1 + (g1 + g2)*h2 - (g1 + g4)*h4)*u1 + ((g1 + g2)*h1 + (g1 + 3*g2)*h2)*u2 - ((g1 + g4)*h1 + (g1 + 3*g4)*h4)*u4)*y1 + (((3*g1 + g2)*h1 + (g1 + g2)*h2)*u1 + ((g1 + g2)*h1 + (g1 - g3)*h2 - (g2 + g3)*h3)*u2 - ((g2 + g3)*h2 + (g2 + 3*g3)*h3)*u3)*y2 + (((3*g2 + g3)*h2 + (g2 + g3)*h3)*u2 + ((g2 + g3)*h2 + (g2 - g4)*h3 - (g3 + g4)*h4)*u3 - ((g3 + g4)*h3 + (g3 + 3*g4)*h4)*u4)*y3 - (((3*g1 + g4)*h1 + (g1 + g4)*h4)*u1 - ((3*g3 + g4)*h3 + (g3 + g4)*h4)*u3 + ((g1 + g4)*h1 - (g3 + g4)*h3 + (g1 - g3)*h4)*u4)*y4);
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \mathbf{\nabla} \cdot (g(\widehat{x},\widehat{y}) \, h(\widehat{x},\widehat{y}) \, \overline{\mathbf{v}})  \big) d\widehat{y}\,d\widehat{x}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: 2D strong convection type integrals
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline double integrate_quadrilateral_grad_dot_h_g_vbar(SVECT *nd, double c, SVECT2D vbar, double *h, double *g) {
    
    double x1, x2, x3, x4, y1, y2, y3, y4, h1, h2, h3, h4, g1, g2, g3, g4;
    extractNodesQuad(nd,&x1,&x2,&x3,&x4,&y1,&y2,&y3,&y4);
    extractFunctionQuad(h,&h1,&h2,&h3,&h4);
    extractFunctionQuad(g,&g1,&g2,&g3,&g4);
    double ub = vbar.x, vb = vbar.y;
    
    double result = one_6 * (((g2 - g4)*h1 + (g1 + 2*g2)*h2 - (g1 + 2*g4)*h4)*vb*x1 - ((2*g1 + g2)*h1 + (g1 - g3)*h2 - (g2 + 2*g3)*h3)*vb*x2 - ((2*g2 + g3)*h2 + (g2 - g4)*h3 - (g3 + 2*g4)*h4)*vb*x3 + ((2*g1 + g4)*h1 - (2*g3 + g4)*h3 + (g1 - g3)*h4)*vb*x4 - ((g2 - g4)*h1 + (g1 + 2*g2)*h2 - (g1 + 2*g4)*h4)*ub*y1 + ((2*g1 + g2)*h1 + (g1 - g3)*h2 - (g2 + 2*g3)*h3)*ub*y2 + ((2*g2 + g3)*h2 + (g2 - g4)*h3 - (g3 + 2*g4)*h4)*ub*y3 - ((2*g1 + g4)*h1 - (2*g3 + g4)*h3 + (g1 - g3)*h4)*ub*y4);
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \mathbf{\nabla}\phi_i(\widehat{x},\widehat{y}) \cdot (\mathbf{D(\mathbf{\nabla}f(\widehat{x},\widehat{y}))}) \big) d\widehat{y}\,d\widehat{x}   \f$
 *  \n where \n
 *  \f$ D_x = \sigma_{xx} \, \frac{\partial f}{\partial x} + \sigma_{xy} \, \frac{\partial f}{\partial y} \f$ \n
 *  \f$ D_y = \sigma_{xy} \, \frac{\partial f}{\partial x} + \sigma_{yy} \, \frac{\partial f}{\partial y} \f$
 *  \author  Corey Trahan, Ph.D.
 ...
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_quadrilateral_gradPhi_dot_Df(SVECT *nd, SQUAD *quad, double c, STENSOR2D T, double *f, double *integral) {
    
    int i, iqp, quad_order = 3; // quadrature order
    SVECT qp_grad_f;
    SQUAD_PT *qp = NULL;
    
    double result[NDONQUAD] = {0., 0., 0., 0.};
    
    for (iqp=0; iqp<quad[quad_order].n; iqp++) {
        qp = &(quad[quad_order].pt[iqp]);
        
        // evaluate djac and cartesian shape function gradients at quadrature point
        ...
        for (i=0; i<NDONQUAD; i++) {
            result[i] += t1 * qp->grad_shp[i].x * (T.xx * qp_grad_f.x + T.xy * qp_grad_f.y);
            result[i] += t1 * qp->grad_shp[i].y * (T.xy * qp_grad_f.x + T.yy * qp_grad_f.y);
        }
    }
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following *vertical quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \phi_i(\widehat{s},\widehat{t}) \big) d\widehat{s}\,d\widehat{t}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: for this integration to be correct, grid must be in column format and numbered correctly.
 *                (i.e. x3 = x2, y3 = y2, x4 = x1, y4 = y1)
 *  \note CJT \:: negative on t1 here because of ordering of nodes
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_quadZ_phi(SVECT *nd, double c, double *integral) {
#if _DEBUG
    assert(nd[0].x == nd[3].x);
    assert(nd[1].x == nd[2].x);
    assert(nd[0].y == nd[3].y);
...
#endif
    double t1 = one_2 * sqrt(pow(nd[0].x - nd[1].x,2) + pow(nd[0].y - nd[1].y,2));
    double t2 = -t1 * one_6 * c;
    integral[0] += t2 * (2*nd[0].z +   nd[1].z -   nd[2].z - 2*nd[3].z);
    integral[1] += t2 * (  nd[0].z + 2*nd[1].z - 2*nd[2].z -   nd[3].z);
    integral[2] += integral[1];
    integral[3] += integral[0];
    
    // old prism code way
//    t1 = -c * one_12 * sqrt(pow(nd[0].x - nd[1].x,2) + pow(nd[0].y - nd[1].y,2));
//    integral[0] = t1*(2*nd[0].z +   nd[1].z -   nd[2].z - 2*nd[3].z);
//    integral[1] = t1*(  nd[0].z + 2*nd[1].z - 2*nd[2].z -   nd[3].z);
//    integral[2] = t1*(  nd[0].z + 2*nd[1].z - 2*nd[2].z -   nd[3].z);
//    integral[3] = t1*(2*nd[0].z +   nd[1].z -   nd[2].z - 2*nd[3].z);
}

/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following *vertical quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \phi_i(\widehat{s},\widehat{t}) \, f(\widehat{s},\widehat{t}) \big) d\widehat{s}\,d\widehat{t}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: for this integration to be correct, grid must be in column format and numbered correctly.
 *                (i.e. x3 = x2, y3 = y2, x4 = x1, y4 = y1)
 *  \note CJT \:: negative on t1 here because of ordering of nodes
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_quadZ_phi_f(SVECT *nd, double c, double *f, double *integral) {
...
#endif
    double t1 = one_2 * sqrt(pow(nd[0].x - nd[1].x,2) + pow(nd[0].y - nd[1].y,2));
    double t2 = -t1 * one_36 * c;
    
    double p1, p2, p3, p4, p5, p6;
    p1 = 2*f[0] + 2*f[1] +   f[2] +   f[3]; p4 =   f[0] +   f[1] + 2*f[2] + 2*f[3];
    p2 = 6*f[0] + 2*f[1] +   f[2] + 3*f[3]; p5 =   f[0] + 3*f[1] + 6*f[2] + 2*f[3];
    p3 = 2*f[0] + 6*f[1] + 3*f[2] +   f[3]; p6 = 3*f[0] +   f[1] + 2*f[2] + 6*f[3];
    
    integral[0] += t2*(p2*nd[0].z + p1*nd[1].z - p1*nd[2].z - p2*nd[3].z);
    integral[1] += t2*(p1*nd[0].z + p3*nd[1].z - p3*nd[2].z - p1*nd[3].z);
    integral[2] += t2*(p4*nd[0].z + p5*nd[1].z - p5*nd[2].z - p4*nd[3].z);
    integral[3] += t2*(p6*nd[0].z + p4*nd[1].z - p4*nd[2].z - p6*nd[3].z);
}

/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*! \brief  Peforms following *vertical quadrilateral integration:
 *  \f$ \int_{-1}^{1} \int_{-1}^{1} \big( c \, \phi_i(\widehat{s},\widehat{t}) \, f(\widehat{s},\widehat{t}) \, g(\widehat{s},\widehat{t})\big) d\widehat{s}\,d\widehat{t}   \f$
 *  \author  Corey Trahan, Ph.D.
 *  \note CJT \:: for this integration to be correct, grid must be in column format and numbered correctly.
 *                (i.e. x3 = x2, y3 = y2, x4 = x1, y4 = y1)
 *  \note CJT \:: negative on t1 here because of ordering of nodes
 */
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
inline void integrate_quadZ_phi_f_g(SVECT *nd, double c, double *f, double *g, double *integral) {
    
//    double t1 = -c * (1./720.) * sqrt(pow(nd[0].x - nd[1].x,2) + pow(nd[0].y - nd[1].y,2));
//    integral[0] =  t1*((3*(12*f[0] + 3*f[1] + f[2] + 4*f[3])*g[0] + (9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[1] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[2] + 3*(4*f[0] + f[1] + f[2] + 4*f[3])*g[3])*nd[0].z + ((9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[0] + (6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[2] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[3])*nd[1].z - ((9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[0] + (6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[2] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[3])*nd[2].z - (3*(12*f[0] + 3*f[1] + f[2] + 4*f[3])*g[0] + (9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[1] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[2] + 3*(4*f[0] + f[1] + f[2] + 4*f[3])*g[3])*nd[3].z);
//    integral[1] =  t1*(((9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[0] + (6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[2] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[3])*nd[0].z + ((6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[0] + 3*(3*f[0] + 12*f[1] + 4*f[2] + f[3])*g[1] + 3*(f[0] + 4*f[1] + 4*f[2] + f[3])*g[2] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[3])*nd[1].z - ((6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[0] + 3*(3*f[0] + 12*f[1] + 4*f[2] + f[3])*g[1] + 3*(f[0] + 4*f[1] + 4*f[2] + f[3])*g[2] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[3])*nd[2].z - ((9*f[0] + 6*f[1] + 2*f[2] + 3*f[3])*g[0] + (6*f[0] + 9*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[2] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[3])*nd[3].z);
//    integral[2] =  t1*(((3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[0] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[2] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[3])*nd[0].z + ((2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[0] + 3*(f[0] + 4*f[1] + 4*f[2] + f[3])*g[1] + 3*(f[0] + 4*f[1] + 12*f[2] + 3*f[3])*g[2] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[3])*nd[1].z - ((2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[0] + 3*(f[0] + 4*f[1] + 4*f[2] + f[3])*g[1] + 3*(f[0] + 4*f[1] + 12*f[2] + 3*f[3])*g[2] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[3])*nd[2].z - ((3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[0] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[2] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[3])*nd[3].z);
//    integral[3] =  t1*((3*(4*f[0] + f[1] + f[2] + 4*f[3])*g[0] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[1] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[2] + 3*(4*f[0] + f[1] + 3*f[2] + 12*f[3])*g[3])*nd[0].z + ((3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[0] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[2] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[3])*nd[1].z - ((3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[0] + (2*f[0] + 3*f[1] + 3*f[2] + 2*f[3])*g[1] + (2*f[0] + 3*f[1] + 9*f[2] + 6*f[3])*g[2] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[3])*nd[2].z - (3*(4*f[0] + f[1] + f[2] + 4*f[3])*g[0] + (3*f[0] + 2*f[1] + 2*f[2] + 3*f[3])*g[1] + (3*f[0] + 2*f[1] + 6*f[2] + 9*f[3])*g[2] + 3*(4*f[0] + f[1] + 3*f[2] + 12*f[3])*g[3])*nd[3].z);
    ...
    double t1 = one_2 * sqrt(pow(nd[0].x - nd[1].x,2) + pow(nd[0].y - nd[1].y,2));
    double t2 = -t1 * one_360 * c;
    
    double ff1, ff2, ff3, ff4, ff5, ff6, ff7, ff8, ff9, ff10, ff11, ff12;
    double gg1, gg2, gg3, gg4, gg5, gg6;
    
    ff1  = (12*f[0] +  3*f[1] +    f[2] + 4*f[3]);  ff2  = (9*f[0] + 6*f[1] + 2*f[2] +  3*f[3]);
    ff3  = ( 3*f[0] +  2*f[1] +  2*f[2] + 3*f[3]);  ff4  = (4*f[0] +   f[1] +   f[2] +  4*f[3]);
    ff5  = ( 6*f[0] +  9*f[1] +  3*f[2] + 2*f[3]);  ff6  = (2*f[0] + 3*f[1] + 3*f[2] +  2*f[3]);
    ff7  = ( 3*f[0] + 12*f[1] +  4*f[2] +   f[3]);  ff8  = (  f[0] + 4*f[1] + 4*f[2] +    f[3]);
    ff9  = ( 2*f[0] +  3*f[1] +  9*f[2] + 6*f[3]);  ff10 = (3*f[0] + 2*f[1] + 6*f[2] +  9*f[3]);
    ff11 = ( f[0] +    4*f[1] + 12*f[2] + 3*f[3]);  ff12 = (4*f[0] +   f[1] + 3*f[2] + 12*f[3]);
    
    gg1 = (3*ff1*g[0] +   ff2*g[1] +   ff3*g[2] + 3*ff4*g[3]); gg2 = (  ff2*g[0] +   ff5*g[1] +    ff6*g[2] +    ff3*g[3]);
    gg3 = (  ff5*g[0] + 3*ff7*g[1] + 3*ff8*g[2] +   ff6*g[3]); gg4 = (  ff6*g[0] + 3*ff8*g[1] + 3*ff11*g[2] +    ff9*g[3]);
    gg5 = (  ff3*g[0] +   ff6*g[1] +   ff9*g[2] +  ff10*g[3]); gg6 = (3*ff4*g[0] +   ff3*g[1] +   ff10*g[2] + 3*ff12*g[3]);
    
    integral[0] +=  t2*(gg1*nd[0].z + gg2*nd[1].z - gg2*nd[2].z - gg1*nd[3].z);
    integral[1] +=  t2*(gg2*nd[0].z + gg3*nd[1].z - gg3*nd[2].z - gg2*nd[3].z);
    integral[2] +=  t2*(gg5*nd[0].z + gg4*nd[1].z - gg4*nd[2].z - gg5*nd[3].z);
    integral[3] +=  t2*(gg6*nd[0].z + gg5*nd[1].z - gg5*nd[2].z - gg6*nd[3].z);
}
