...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*!
 *  \brief     Returns the SUPG tau parameter for a general A/D equation
 *  \author    Corey Trahan, Ph.D.
 *  \bug       none
 *  \warning   none
 *  \copyright AdH
 ...
 * @param[in] nnodes (int) the number of nodes on the element
 * @param[in] nodes (SVECT *) the elemental node locations
 * @param[in] diffusion (double) the elemental viscosity
 * @param[in] elem_avg_u (double) the elementally averaged x-velocity
 * @param[in] elem_avg_v (double) the elementally averaged y-velocity
 * @param[in] elem_avg_w (double) the elementally averaged z-velocity
 * @param[in] grad_shp_x (double *) the x-gradient in cartesian space of the basis functions
...
double fe_get_supg_tau(int nnodes, SVECT *node, double diffusion, double elem_avg_u, double elem_avg_v,
    ...
    double flow_norm = 0;
    if (ndim == 1) {
        flow_norm = sqrt(elem_avg_u*elem_avg_u);
    } else if (ndim == 2) {
        flow_norm = sqrt(elem_avg_u*elem_avg_u + elem_avg_v*elem_avg_v);
    } else {
        flow_norm = sqrt(elem_avg_u*elem_avg_u + elem_avg_v*elem_avg_v + elem_avg_w*elem_avg_w);
    }
    
    
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    // Method = 0 :: Standard Galerkin FEM
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    
    if (tau_method_flag == 0) {
        return 0.0;
    }
    
    /* Pick ``Element length'' in the direction of flow, Eq. (3.13) */
    double le = get_element_length(nnodes, node, elem_avg_u, elem_avg_v, elem_avg_w, grad_shp_x, grad_shp_y,
                                   grad_shp_z, djac, ndim, le_method_flag); // UNITS: [L]
    
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    // Method = 1 :: Tezduyar and Park
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    
    if (tau_method_flag == 1) {
        
        /* Element Peclet number, Eq. (3.14) */
        // UNITS: viscosity = [L]^2/[T]
        double alphaR = (one_2 * flow_norm * le) / (diffusion);  // UNITS: none
        
        /* Velocity weighting to SUPG, Eq. (3.11) */
        double xi = 1.0;   // UNITS: none
        if (alphaR <= 3) xi = alphaR / 3.0;
        
        /* Multiplicative factor, Eq. (3.9) & Eq. (3.10) = (1/2) (h) (xi) */
        double tau = le * xi / (flow_norm + 2. * NOT_QUITE_SMALL);  // UNITS = [L] / ([L]/[T]) = [T]
      
        if (tau < 1e-6) tau = 1e-5;
        
#if _DEBUG
        if (tau < 1e-6) {
            printf("Error (file:line) %s:%d\n", __FILE__, __LINE__);
            printf("tau: %20.10e \t le: %20.10e \t flow_norm: %20.10e \t xi: %20.10e \t diffusion: %20.10e \n",tau,le,flow_norm,xi,diffusion);
            tl_error(">> tau calculation is <= 0\n");
        }
#endif
    ...
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    // Method = 2 :: Old AdH way
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    
    if (tau_method_flag == 2) {

        double tau = le / (flow_norm + 2. * NOT_QUITE_SMALL); // UNITS = [L] / ([L]/[T]) = [T]
#if _DEBUG
        if (tau < 1e-6) {
            printf("Error (file:line) %s:%d\n", __FILE__, __LINE__);
            printf("le: %20.10e \t flow_norm: %20.10e\n",le,flow_norm);
            tl_error(">> tau calculation is <= 0\n");
        }
#endif
