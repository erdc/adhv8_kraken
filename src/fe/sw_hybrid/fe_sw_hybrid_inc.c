...
void fe_sw_hybrid_inc(SSUPER_MODEL *sm, int idum) {
#ifndef _PETSC
    int i;	    		/* loop counter */
    SMODEL *mod;                /* alias */
...
#ifdef _DEBUG
                if (DEBUG){
                    printf("\n2D Model %i solution (HVEL stage):", i+1);
                    int j;
                    for (j = 0; j < mod->grid->nnodes; j++) {
                        printf("\n    Node % 5i: vel.x = % 9.6e\t vel.y = % 9.6e\t depth = % 9.6e", j+1, mod->sw->d2->vel[j].x, mod->sw->d2->vel[j].y, mod->sw->d2->head[j]);
                    }
                    printf("\n");
                }
#endif
...
#ifdef _DEBUG
                if (DEBUG){
                    printf("\n3D Model %i solution (HVEL stage):", i+1);
                    int j;
                    for (j = 0; j < mod->grid->nnodes; j++) {
                        printf("\n    Node % 5i: vel.x = % 9.6e\t vel.y = % 9.6e\t displ = % 9.6e", j+1, mod->sw->d3->vel[j].x, mod->sw->d3->vel[j].y, mod->sw->d3->displacement[j]);
                    }
                    printf("\n");
                }
#endif
            ...
            if (sm->submodel[ifce->model_id[0]].flag.SW2_FLOW){
                if(sm->submodel[ifce->model_id[1]].flag.SW2_FLOW){   /* 2D 2D coupling */
                    for (k=0; k<ifce->NumNodeColumns; k++){
                        ...
                        printf("\nColumn %i :",k+1);
                        printf("\n  Model 1 (2D) :");
                        printf("\n    Node % 5i : vel.x = % 9.6e\t vel.y = % 9.6e\t depth = % 9.6e",
                    ...
                    for (k=0; k<ifce->NumNodeColumns; k++){
                        ...
                        //                        for (l=0; l<ndlist->size[1]; l++){
                        //                            printf("\n    Node % 5i : vel.x = % 9.6e\t vel.y = % 9.6e\t displ = % 9.6e",
                        //                                   ndlist->couplednodes[1][l]+1,
                        //                                   mod2->sw->d3->vel[ndlist->couplednodes[1][l]].x,
                        //                                   mod2->sw->d3->vel[ndlist->couplednodes[1][l]].y,
                        //                                   mod2->sw->d3->displacement[ndlist->couplednodes[1][l]]
