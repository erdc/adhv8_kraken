/* This routine loops over all submodels, calling functions that set boundary conditions for transport */
...
void fe_transport_hybrid_init(SSUPER_MODEL *sm, int dum) {
    int i;	    		/* loop counter */
    int j, k, l;
    int imodel_id;
    //int node_count = 0;
    ...
    for (j=0; j<sm->NumInterfaces; j++){
        ...
        for (imodel_id=0; imodel_id<2; imodel_id++){
            ...
            //mod->bc_mask = &(sm->bc_mask[node_count*sm->nsys]);
            if(mod->proc_flag==1){
...
#endif
                printScreen_int_array("bc_mask", sm->bc_mask, sm->total_nnodes*sm->nsys, __LINE__, __FILE__);
#ifdef _MESSG
            }
