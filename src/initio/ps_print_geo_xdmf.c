...
)
{
    ...
    if (grid->ndim == 3) {
	nelem = nelem3d;
	if (grid->haveTets == YES) nodes_per_elem = NDONTET;
	if (grid->havePrisms == YES) nodes_per_elem = NDONPRISM;
    } else {
	nelem = nelem2d;
        if (grid->haveTris == YES) nodes_per_elem = NDONTRI;
        if (grid->haveQuads == YES) nodes_per_elem = NDONQUAD;
    }
    ...
    if (grid->ndim == 3) {
        ...
        for (ielem = 0; ielem < nelem; ielem++) {
            owner = npes;
            for (jj = 0; jj < grid->elem3d[ielem].nnodes; jj++) {
        ...
        for (ielem = 0; ielem < nelem; ielem++) {
            owner = npes;
            for (jj = 0; jj < grid->elem2d[ielem].nnodes; jj++) {
