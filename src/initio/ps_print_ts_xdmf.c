...
)
{
    ...
    write_node_index = (int *)tl_alloc(sizeof(int), nsize);
    ...
    //write_node_index = (int *)tl_alloc(sizeof(int), nnodes);
    
    //for (i = 0; i < nnodes; i++)
    //  {
    // /*   if (orig_nd_number[i] != UNSET_INT)
    //      {
    //  */
    //        write_node_index[nsize]=i;
    //        nsize++;
    //  /*    } */
    //  }
    ...
    if (data_type == SCALAR_DATA)
    {
        ...
        vector_data = (double *)tl_alloc(sizeof(double), nsize);
        for (i = 0; i < nsize; i++)
        {
