...
# Basic test suite test definitions for the Pre-AdH Executable
#=============================================================================
set(exec_name "pre-AdH")

# Local macro to wrap and simpifly function call
macro(myAddExecTest _name _exec_arg _timeout)
  AddExecTest(PRE "${_name}" "" BASIC "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}" 0
    "${_exec_arg}" ${_timeout}) 
endmacro()
macro(myTestOptionEnabled _name _rgx _opt)
  TestOptionEnabled(PRE "${exec_name}_${_name}" ${_rgx} ${_opt})
endmacro()

##############################################################################
##############################################################################
# The following tests ensure that Pre-AdH runs and provides screen output
# (stdout) for the user.
# They do not necessarily test the results.

##############################################################################
# Was executable built?

TestOutputCompliance(${exec_name} EXEC TRUE pre_adh pre_${DASHBOARD_LABEL})

##############################################################################
# Does Pre-AdH warn about missing command line agrument?
...
##############################################################################
# Does Pre-AdH provide version information with -v argument?
...
set(rgx_bi "${rgx_bi}---*${rgx_nl}")
set(rgx_bi "${rgx_bi}SVN revision # [0-9:MSP]+${rgx_nl}")
set(rgx_bi "${rgx_bi}Build Date/Time: 20[0-9][0-9]\\.[01][0-9]\\.[0-3][0-9] / [0-2][0-9]:[0-5][0-9]:[0-5][0-9]${rgx_nl}")
...
##############################################################################
# Does Pre-AdH list specific build options?
set(opt_on ON)
set(opt_off OFF)
...
set(rgx "XMS \\(\\*\\.dat\\) output file format")
...
# The following options should not be enabled for Pre-Adh
set(rgx "MPI enabled")
myTestOptionEnabled("MPI" rgx opt_off)

set(rgx "PARMETIS enabled")
myTestOptionEnabled("PARMETIS" rgx opt_off)

set(rgx "UMFPACK (enabled|Version [345] \\(int size: (32|64) bytes)")
myTestOptionEnabled("UMFPACK" rgx opt_off)

##############################################################################
# Does Pre-AdH provide information as screen output?

set(test_name ${exec_name}_Screen_Output)
# Initially call Pre-AdH with bad input argument
...
#-----------------------------------------------------------------------------
# 2. Check for runtime information section
set(test_num 2)
# Create regex of expected output (done in parts for readability)
set(rgx       "-------------------${rgx_nl}")
...
#-----------------------------------------------------------------------------
# 3. Check for run files section (default options - no super file)
set(test_num 3)
# Create regex of expected output (done in parts for readability)
set(rgx       "---------${rgx_nl}")
set(rgx "${rgx}Run Files${rgx_nl}")
set(rgx "${rgx}---------${rgx_nl}")
set(rgx "${rgx}Geometry file: ${exec_arg}.3dm - NOT FOUND${rgx_nl}")
set(rgx "${rgx}Boundary condition \\(BC\\) file: ${exec_arg}.bc - NOT FOUND${rgx_nl}")
set(rgx "${rgx}Hotstart file: ${exec_arg}.hot - NOT FOUND${rgx_nl}")
set(rgx "${rgx}Optional input files \\(dependent on inclusion of OP cards in BC file\\):${rgx_nl}")
set(rgx "${rgx}Vessel file: ${exec_arg}.bt - Used only if OP BT or OP BTS is specified${rgx_nl}")
if(PHYSICS_HEAT)
  set(rgx "${rgx}Meteorological file: ${exec_arg}.met - Used only if OP MET is specified${rgx_nl}")
endif(PHYSICS_HEAT)
set(rgx "${rgx}${rgx_nl}")
set(rgx "${rgx}ERROR: Could not find the above file\\(s\\).${rgx_nl}")
...
#-----------------------------------------------------------------------------
# 4. Check for exit error from bad input
set(test_num 4)
