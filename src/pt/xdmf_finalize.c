...
#ifdef _ADH_HDF5
void xdmf_finalize(HDF5 *hdf5, const char *proj_name, int npes, int myid, int flag)
{
    herr_t status;
    //xmlDtdPtr dtd = NULL;
    ...
    for (igrid = 0; igrid < npes; igrid++)  {
        ...
        if (xpathCtx == NULL)
        {
            ...
            fprintf(stderr,"XDMF Finalize Error: unable to create new XPath context; %s:%d\n", __FILE__, __LINE__);
            xmlFree(doc_out);
            exit(0);
        }
    ...
    /* Create Master file */
    sprintf(filename,"%s_pall_%d.xmf", proj_name, igrid);
