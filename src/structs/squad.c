...
int SQUAD_triangle_alloc_init(SQUAD **quad_tri) {
    ...
    for (iorder=0; iorder<=MAX_QUAD_ORDER; iorder++) {
        for (iqp=0; iqp<qp_tri[iorder].n; iqp++) {
            ...
            get_triangle_local_shape(qp_tri[iorder].pt[iqp].xhat, qp_tri[iorder].pt[iqp].yhat, qp_tri[iorder].pt[iqp].zhat, qp_tri[iorder].pt[iqp].lshape);
            
            // calculate local element quadratic shape values at quad points
            get_triangle_local_shape_quad(qp_tri[iorder].pt[iqp].xhat, qp_tri[iorder].pt[iqp].yhat, qp_tri[iorder].pt[iqp].zhat, qp_tri[iorder].pt[iqp].lshape_quad);
        }
    }
...
int SQUAD_rectangle_alloc_init(SQUAD **quad_rect) {
    ...
    for (iorder=0; iorder<=MAX_QUAD_ORDER; iorder++) {
        for (iqp=0; iqp<qp_rect[iorder].n; iqp++) {
            // calculate local element shape values at quad points
            get_quadrilateral_local_shape(qp_rect[iorder].pt[iqp].xhat, qp_rect[iorder].pt[iqp].yhat, qp_rect[iorder].pt[iqp].zhat, qp_rect[iorder].pt[iqp].lshape);
            
            // calculate local element quadratic shape values at quad points
            get_quadrilateral_local_shape_quad(qp_rect[iorder].pt[iqp].xhat, qp_rect[iorder].pt[iqp].yhat, qp_rect[iorder].pt[iqp].zhat, qp_rect[iorder].pt[iqp].lshape_quad);
        }
    }
...
int SQUAD_tetrahedron_alloc_init(SQUAD **quad_tet) {
    ...
    for (iorder=0; iorder<=MAX_QUAD_ORDER; iorder++) {
        for (iqp=0; iqp<qp_tet[iorder].n; iqp++) {
            // calculate local element shape values at quad points
            get_tet_local_shape(qp_tet[iorder].pt[iqp].xhat, qp_tet[iorder].pt[iqp].yhat, qp_tet[iorder].pt[iqp].zhat, qp_tet[iorder].pt[iqp].lshape);
            
            // calculate local element quadratic shape values at quad points
            get_tet_local_shape_quad(qp_tet[iorder].pt[iqp].xhat, qp_tet[iorder].pt[iqp].yhat, qp_tet[iorder].pt[iqp].zhat, qp_tet[iorder].pt[iqp].lshape_quad);
        }
    }
...
//    qp_prism[4].pt[20].xhat = xquad_tri_4_7; qp_prism[4].pt[20].yhat = yquad_tri_4_7; qp_prism[4].pt[20].zhat = xquad_line_45_3; qp_prism[4].pt[20].w = wquad_line_45_3*wquad_tri_4_7;
    ...
    for (iorder=0; iorder<=MAX_QUAD_ORDER; iorder++) {
        for (iqp=0; iqp<qp_prism[iorder].n; iqp++) {
            // calculate local element shape values at quad points
            get_triprism_local_shape(qp_prism[iorder].pt[iqp].xhat, qp_prism[iorder].pt[iqp].yhat, qp_prism[iorder].pt[iqp].zhat, qp_prism[iorder].pt[iqp].lshape);
            
            // calculate local element quadratic shape values at quad points
            get_triprism_local_shape_quad(qp_prism[iorder].pt[iqp].xhat, qp_prism[iorder].pt[iqp].yhat, qp_prism[iorder].pt[iqp].zhat, qp_prism[iorder].pt[iqp].lshape_quad);
        }
    }
