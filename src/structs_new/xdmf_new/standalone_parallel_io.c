...
write_hdf5_data(MPI_Comm comm, MPI_Info info, int mpi_rank, int mpi_size)
{
    ...
    //create file
    file_id = H5Fcreate("Parallel_dataset.h5", H5F_ACC_TRUNC, H5P_DEFAULT, plist_id);
    H5Pclose(plist_id);


    //create a dataspace
    // this is a global 8x5 matrix
    dims[0]  = 8;
    dims[1]  = 5;
    filespace = H5Screate_simple(RANK, dims, NULL);

    //create a parallel dataset object and close filespace
    dset_id = H5Dcreate(file_id, "IntArray", H5T_NATIVE_INT, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    H5Sclose(filespace);
